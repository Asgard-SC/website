        ...
        switch (event.key) {
          ...
          case 'Enter':
            if (results.value.length > 0) {
              navigate(`/${(results.value[active.value] as any).collection}/${(results.value[active.value] as any).slug}`);
            }
            break;
        
        }
        if (prevActive !== active.value) {
          const element = document.getElementById(`search-${(results.value[active.value] as any).collection}-${(results.value[active.value] as any).slug}`);
